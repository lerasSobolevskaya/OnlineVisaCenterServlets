...
<body
	...
	<div class="container" id="wrapper">
		...
		<form action="MainServlet" method="get">
			<input type="hidden" name="command" value="show_visa_by_status" /> <select
				id="Visas-selected-deposit-country" name="visa_status" onchange="">
				<c:if test="${not empty visa_status}">
					<option selected>
						<c:out value="${visa_status}" />
				</c:if>
				<c:forEach var="status" items="${visa_status_list}">
					<option value="${status}">${status}</option>
				</c:forEach>
			</select> <input type="submit"
				value="<fmt:message key="show_visa_by_status"/>"
					...
					<c:forEach items="${visas}" var="visa">
						<tr>
							<th class="text-center align-middle"><a
								href="/OnlineVisaCenter/MainServlet?command=visa_details_by_id&visa_id=${visa.getId()}">Детали</a></th>

							<td class="text-center align-middle"><c:out
									value="${visa.getSchengenCountries()}" /></td>

							<td class="text-center align-middle"><c:out
									value="${visa.getVisaStatus()}" /></td>
